...
export function PublishScreen({ onDone, onCancel }: { onDone: () => void; onCancel: () => void }) {
  ...
  return (
    <div className="screen pt-3">
      <Button variant="outline" size="icon" className="mb-4" onClick={onCancel} aria-label="Annuler">
        <ChevronLeft className="h-5 w-5" />
      </Button>
      <form onSubmit={submit} className="space-y-5">
        <div className="space-y-2">
          <Label>Titre de l’annonce *</Label>
          <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Ex. Report de l’examen de statistiques" maxLength={120} />
        </div>
        <div className="space-y-2">
          <Label>Type</Label>
          <div className="grid grid-cols-2 gap-2">
            {TYPES.map((t) => (
              <button
                type="button"
                key={t}
                className={cn("rounded-xl border px-3 py-3 text-[13.5px] font-semibold", type === t ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground")}
                onClick={() => setType(t)}
              >
                {TYPE_INFO[t].label}
              </button>
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button type="button" className={cn("rounded-xl border py-3 font-semibold", priority === "NORMALE" ? "border-primary text-primary" : "border-border")} onClick={() => setPriority("NORMALE")}>
              Normale
            </button>
            <button
              type="button"
              disabled={!urgente}
              className={cn("rounded-xl border py-3 font-semibold disabled:opacity-40", priority === "URGENTE" ? "border-red-400 text-red-400" : "border-border")}
